      ...
      <div class="chip-wrapper">
        <button class="chip" [class]="'chip--' + state()" (click)="popoverOpen.set(!popoverOpen())">
          ...
          @switch (state()) {
            ...
            @case ('loading') { <span>loading · {{ (loadProgress() * 100).toFixed(0) 
  }}%</span> }
            @case ('ready')   { <span>local · WebGPU · {{ tokensPerSec() }} tok/s</span> 
  }
        ...
        @if (popoverOpen()) {
          <div class="popover">
            ...
            <div class="popover-row"><span>Speed</span><span>{{ tokensPerSec() }}
  tok/s</span></div>
            @if (errorMessage()) {
